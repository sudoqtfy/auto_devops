...
//        $(this).prop('disabled', true);
    ...
    function modalAllowHandler(event) {
        var form = $(this).parents('.modal-content').find('form[role="allow"]');
        $.post(form.attr('action'), form.serialize(), modalAjaxHandler(form));
    }


    function modalRejectHandler(event) {
        var form = $(this).parents('.modal-content').find('form[role="reject"]');
        $.post(form.attr('action'), form.serialize(), modalAjaxHandler(form));
    }

    body.on('click', '.modal .save', modalSaveHandler);
    body.on('click', '.modal .allow', modalAllowHandler);
    body.on('click', '.modal .reject', modalRejectHandler);
